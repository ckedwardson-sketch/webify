...
// ---------------------------------------------------------------------------
// Tab 2 — Automatic cloud backups (scheduled rotated snapshots)
// ---------------------------------------------------------------------------

function CloudBackupsTab() {
    ...
    maybeRunAutomaticBackup()
      .then((r) => {
        if (r) {
          ...
          return fetchBackupSettings().then((s) => {
            skipNextSave.current = true;
            setSettings(s);
            return refreshList();
          });
  ...
  return (
    <div className="settings-groups">
      ...
      <p className="page-text">
        Scheduled snapshots of this device's database, kept in a rotating set on the device.
        Use these as a safety net alongside wifi sync — they do not replace the computer copy.
        (A future release can push the same snapshots to a personal cloud folder; the settings
        below already control size, frequency, and retention.)
      </p>
...
export function SettingsSyncPage({
  ...
  return (
    <div className="page">
      ...
      {active === "sync" && <SyncTab />}
      {active === "cloud-backups" && <CloudBackupsTab />}
