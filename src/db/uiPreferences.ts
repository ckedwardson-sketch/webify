...
// default, notes-tree expand memory, dreamWebFullView) rather than one
...
export async function fetchUiPreferences(): Promise<Record<string, string>> {
  ...
  const rows = await db.select<{ key: string; value: string }[]>("SELECT key, value FROM ui_preferences");
